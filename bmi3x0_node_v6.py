...
from sensor_msgs.msg import Imu, MagneticField, Image, CompressedImage
...
class BMI3x0ReceiverNode:
    def __init__(self):
        ...
        self.gps_pub = rospy.Publisher("/mavros/gps_input/gps_input", GPSINPUT, queue_size=10)
    ...
    def telemetry_loop(self):
        ...
        GPS_PAYLOAD_SIZE = 81  # Corrected: 84 total - 3 header bytes = 81 bytes
        ...
        while not rospy.is_shutdown() and self.running:
            ...
            try:
                ...
                while not rospy.is_shutdown() and self.running:
                    ...
                    elif packet_type == 0x02:  # GPSINPUT
                ...
                rospy.logwarn_throttle(2, f"[Telemetry] Connection lost: {e}. Retrying in 2s...")
                time.sleep(2)
            finally:
                if sock:
                    sock.close()
    ...
    def process_gps_packet(self, payload):
        # Format matching packet_types.h (after sync[2] & type[1]):
    ...
    def camera_loop(self):
        ...
        while not rospy.is_shutdown() and self.running:
            ...
            try:
                ...
                rospy.logwarn_throttle(2, f"[Camera] Connection lost: {e}. Retrying in 2s...")
                time.sleep(2)
            finally:
                if sock:
                    sock.close()
