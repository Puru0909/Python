...
class BMI3x0ReceiverNode:
    ...
    def process_imu_packet(self, payload):
        data = struct.unpack("<q 3f 3f 3f 4f", payload)
        timestamp_ns, accel, gyro, mag, quat = data[0], data[1:4], data[4:7], data[7:10], data[10:14]
        ...
        mag_msg.header.frame_id = self.frame_id_imu
        ...
        while not rospy.is_shutdown() and self.running:
            try:
                ...
                while not rospy.is_shutdown() and self.running:
                    ...
                    # Payload is now compressed JPEG bytes directly from C++ daemon
